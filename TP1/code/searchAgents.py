...
class CornersProblem(search.SearchProblem):
    ...
    def _set_corner_values(self, state):
        try:
            ...
            updated_corners = list(state[1])
            updated_corners[index] = True
            state = (state[0], tuple(updated_corners))
        except ValueError:
            pass
        finally:
            return state
        ...
        for action in [
            ...
            if not hitsWall:
                ...
                if next in self.corners and next not in visited_corners:
                    visited_corners.append(next)

                new_state = (next, tuple(visited_corners))
                succ = (new_state, action, 1)
...
def cornersHeuristic(state, problem):
    ...
    coords, visited_corners = state[0], state[1]
    must_visit_corners = list()

    for corner in corners:
        if corner not in visited_corners:
            must_visit_corners.append(corner)

    sum = 0
    while len(must_visit_corners) != 0:
        weights = []
        for corner in must_visit_corners:
            weights.append((corner, util.manhattanDistance(coords, corner)))

        min_corner = min(weights, key=lambda x: x[1])
        must_visit_corners.remove(min_corner[0])
...
def foodHeuristic(state, problem: FoodSearchProblem):
    ...
    for food in foodGrid.asList():
        distance_list.append((food, util.manhattanDistance(position, food)))

    if len(distance_list) == 0:
        return 0

    min_food_dist = min(distance_list, key=lambda x: x[1])[0]
    max_food_dist = max(distance_list, key=lambda x: x[1])[0]

    return util.manhattanDistance(position, min_food_dist) + util.manhattanDistance(min_food_dist, max_food_dist)
